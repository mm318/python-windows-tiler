...
class Monitor(object):
    def __init__(self, hMonitor):
        ...
        self.tilers = []
        for i in range(9):
            tiler_name = 'Monitor %d Group %d' % (self.hMonitor, i+1)
            self.tilers.append(Tiler(tiler_name, self.workarea))
    ...
    def contains_window(self, window):
        ...
        try:
            if(window == None):
                return False
            if(self.hMonitor == win32api.MonitorFromWindow(window.hWindow, MONITOR_DEFAULTTONEAREST)):
                return True
            else:
                return False
    ...
    def current_monitor_from_list(monitors):
        return Monitor.monitor_from_point_in_list(monitors, win32api.GetCursorPos())
